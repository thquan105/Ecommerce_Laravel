                                        ...
                                        if ($maxPrice <= $attribute->data()['price']) {
                                            ...
                                            <td>
                                                ...
                                                <a href="{{ $item->data()['url'] }}" target="_blank">
                                                    <img src="{{ $item->data()['url'] }}" width="45px" height="45px" alt="">
                                                </a>
                                                @endforeach
                                                @else
                                            ...
                                            <td>
                                                <div class="btn-group btn-group-sm">
                                                    <a href="{{ route('seller.products.edit', $product->id()) }}" class="btn btn-sm btn-primary mr-1">
                                                        <i class="fa fa-edit">{{ __(' Sửa') }}</i>
                                                    </a>
                                                    <form onclick="return confirm('Chắc chắn xóa ?')" action="{{ route('seller.products.destroy', $product->id()) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button class="btn btn-sm btn-danger" type="submit"><i class="fa fa-trash">{{ __(' Xóa') }}</i></button>
                                                    </form>
                                                </div>
                                            </td>
                                    </tr>
